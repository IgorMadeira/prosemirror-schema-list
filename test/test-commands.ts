import {
  ...
  NodeSelection,
  Command,
} from "@igormadeira/prosemirror-state";
import {
  schema,
  eq,
  doc,
  ...
  ol,
  ul,
} from "@igormadeira/prosemirror-test-builder";
import ist from "ist";
import {
  wrapInList,
  splitListItem,
  liftListItem,
  sinkListItem,
} from "@igormadeira/prosemirror-schema-list";
import { Node } from "@igormadeira/prosemirror-model";
...
describe("liftListItem", () => {
  ...
  it("can lift two items from a nested three-item list", () =>
    apply(
      doc(
        ul(li(p("hello"), ul(li(p("o<a>ne")), li(p("two<b>")), li(p("three")))))
      ),
      lift,
